        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-center mb-8">
            ...
            <span className="text-sm text-gray-500 bg-white px-3 py-1.5 rounded-full border border-gray-200">
              {templates?.phone_agents?.length} templates available
            </span>
          </div>
            ...
            <div className="flex flex-wrap gap-6">
              <AnimatePresence mode="wait">
                {isLoading
                  ...
                  : templates?.phone_agents?.map((template) => (
                      <motion.div
                        key={template.id}
                      ...
                      >
                            ...
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-medium ${
                                template.type === "outbound"
                                  ? "bg-[#13104A]/10 text-[#13104A]"
                                  : "bg-[#2D3377]/10 text-[#2D3377]"
                              }`}
                            >
                              {template.type}
                            </span>
                        ...
                        <div className="mt-auto">
                          <button
                            onClick={() =>
                              router.push(
                                `/workspace/agents/phone/createagent?template=${encodeURIComponent(JSON.stringify(template))}`
                              )
                            }
